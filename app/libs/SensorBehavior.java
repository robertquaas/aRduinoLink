...
public class SensorBehavior implements Parcelable {
    /**
     * A PendingIntent that the Science Journal app can send to open an activity that will
     * allow the user to change settings for this particular sensor.  Be sure that different
     * sensors issue different settingsIntents (simply changing the extras is not enough, see the
     * javadoc for PendingIntent.)
     */
    public PendingIntent settingsIntent = null;

    /**
     * If your sensor can be connected to meaningfully without user input, set this to false, even
     * if you have a settings screen.  For example, a thermometer can default to Celsius, and users
     * can choose to open the settings screen to switch to Fahrenheit.
     *
     * However, if there is no sensible default behavior (for example, this is a pH meter that
     * emits nonsense until calibrated), leave this true.
     */
    public boolean shouldShowSettingsOnConnect = true;

    /**
     * An id that will be used to log usage and problem reports.  All reports will already contain
     * your service's package name.  The loggingId might contain additional information about the
     * _type_ of sensor being used ("thermometer"), but should not contain anything that would allow
     * individual users to be identified (no user names or physical or virtual addresses of any
     * kind)
     */
